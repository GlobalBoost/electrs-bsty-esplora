    ...
    pub fn new(
        ...
        let daemon = Daemon {
            ...
            latency: metrics.histogram_vec(
                HistogramOpts::new("daemon_rpc", "GlobalBoostd RPC latency (in seconds)"),
                &["method"],
            ),
            size: metrics.histogram_vec(
                HistogramOpts::new("daemon_bytes", "GlobalBoostd RPC size (in bytes)"),
                &["method", "dir"],
            ),
        };
        ...
        if network_info.version < 16_00_00 {
            bail!(
                "{} is not supported - please use globalboostd 0.16+",
        ...
        if blockchain_info.pruned {
            bail!("pruned node is not supported (use '-prune=0' globalboostd flag)".to_owned())
        }
        loop {
            ...
            warn!(
                "waiting for globalboostd sync to finish: {}/{} blocks, verification progress: {:.3}%",
                info.blocks,
                info.headers,
        ...
        loop {
            match self.handle_request_batch(method, params_list) {
                Err(Error(ErrorKind::Connection(msg), _)) => {
                    warn!("reconnecting to globalboostd: {}", msg);
