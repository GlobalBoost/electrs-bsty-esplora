...
[latest GlobalBoost Core](https://bsty.org) (0.16+)
and [latest Electrum-BSTY wallet](https://bsty.org/#download) (3.2+).
...
## globalboostd configuration

Allow GlobalBoost daemon to sync before starting Electrum server:
```bash
$ globalboostd -server=1 -txindex=0 -prune=0
```
...
Otherwise, [`~/.globalboost/.cookie`](https://github.com/bitcoin/bitcoin/blob/0212187fc624ea4a02fc99bc57ebd413499a9ee1/contrib/debian/examples/bitcoin.conf#L70-L72) will be read, allowing this server to use globalboostd JSONRPC interface.
